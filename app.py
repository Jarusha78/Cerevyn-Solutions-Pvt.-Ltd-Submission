...
COLOR_WORDS = {
    'black','white','red','blue','gre   en','yellow','pink','orange','purple',
...
# NEW: Logs directory for query JSONL logs
...
def save_query_log(entry: dict):
    """
    Maintain a JSON array per day:
    logs/query_log_YYYY-MM-DD.json
    """
    try:
        day_str = datetime.utcnow().strftime("%Y-%m-%d")
        log_path = os.path.join(LOG_DIR, f"query_log_{day_str}.json")
        ...
        # Write back pretty-printed JSON array
...
def ws_query_with_inputs(data):
    ...
    try:
        ...
        error_entry = {
            ...
            "sid": sid,
            "request_id": request_id,
            "input": safe_jsonify(data) if isinstance(data, dict) else str(data),
            "error": str(e),
            "trace": traceback.format_exc()
        }
        save_query_log(error_entry)
